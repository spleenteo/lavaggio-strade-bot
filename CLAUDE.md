...
- `.github/workflows/refresh-data.yml` rigenera e ricarica i dati su KV ogni lunedì (04:30 UTC) e si può lanciare a mano dal tab Actions; richiede i secret `CLOUDFLARE_API_TOKEN` (Workers KV Storage: Edit) e `CLOUDFLARE_ACCOUNT_ID` nel repo GitHub. `.dev.vars.example` è il template per `.dev.vars` (locale, gitignored).
